...
from secrets import choice #Could use random choices but exercise said not to
...
def gen_key(parts=4, chars_per_part=8):
    ...
    for n in range(parts):
        part = ''.join(choice(pool) for i in range(chars_per_part))
        #belows adds the hypen in the correct place given the length of the key
        if n < (parts - 1):
            key += part + '-'
        else:
            key += part
